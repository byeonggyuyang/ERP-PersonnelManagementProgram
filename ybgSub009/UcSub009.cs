...
namespace ybgSub009
{
    public partial class UserControl1: UserControl
    {
        ...
        public void BtnSearch2_Click()
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["car_yyyymm_f"].Value = date1;
                    row.Cells["car_yyyymm_t"].Value = date2;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    cmd.Parameters.Add("car_yyyymm_f", OracleDbType.Varchar2).Value = (row.Cells["car_yyyymm_f"].Value.ToString().Replace("-", ""));
                    cmd.Parameters.Add("car_yyyymm_t", OracleDbType.Varchar2).Value = (row.Cells["car_yyyymm_t"].Value.ToString().Replace("-", ""));
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_YYYYMM(ct_car_yyyymm_f.Text) == false)
            {
                SetError(ct_car_yyyymm_f, "근무시작월 날짜형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_car_yyyymm_f, errorProvider1);
            }
            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_YYYYMM(ct_car_yyyymm_t.Text) == false)
            {
                SetError(ct_car_yyyymm_t, "근무종료월 날짜형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_car_yyyymm_t, errorProvider1);
            }
            //*---------------------------------------------------------------------------------------------------------
            if (ct_car_pos.Text == "")
            {
                SetError(ct_car_pos, "최종직급을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_car_pos, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (ct_car_dept.Text == "")
            {
                SetError(ct_car_dept, "담당부서를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_car_dept, errorProvider1);
            }
            //*---------------------------------------------------------------------------------------------------------
        }
