...
namespace ybgSub022
{
    public partial class UserControl1: UserControl
    {
        ...
        {
            if (MessageBox.Show(temp + " " + temp2 + " 님의 " + "재직증명서를 발급하시겠습니까?", "주의", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                seq = null;
                //--DB Handling(Start)-------------------------------------
                try
                {
                    ...
                    if (dr.Read())
                    {
                        ...
                        ymd = ymd.Substring(0, 4) + "년 " + ymd.Substring(5, 2) + "월 " + ymd.Substring(8, 2) + "일";
                        reportViewer1.LocalReport.SetParameters(new ReportParameter("today", ymd));
                        reportViewer1.LocalReport.SetParameters(new ReportParameter("ceri_no", dr["ceri_no"].ToString()));
                        seq = dr["ceri_no"].ToString();
                    }
                    dr.Close();
                    insertCeri();
                    this.reportViewer1.RefreshReport();
        ...
        public void insertCeri() 
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("ceri_kind", OracleDbType.Varchar2).Value = "재직";
                cmd.Parameters.Add("ceri_date", OracleDbType.Varchar2).Value = DateTime.Now.ToString("yyyyMMdd");
                cmd.Parameters.Add("ceri_lang", OracleDbType.Varchar2).Value = "국문";
