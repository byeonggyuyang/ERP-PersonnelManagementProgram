...
namespace ybgSub024
{
    class SQLStatement
    {
        public static string
          SelectSQL = @"SELECT POS.CD_CODNMS pos_name, DUT.CD_CODNMS dut_name, dept.dept_name dept_name, 
                        POS.CD_ENGNM pos_ename, DUT.CD_ENGNM dut_ename, dept.dept_eng dept_ename, 
                        STS.CD_CODNMS sts_name, dept.dept_code, BAS.* FROM ybgTINSA_BAS BAS
        ...
        public static string
            SelectSQL0 = @"select bas.bas_ename, bas.bas_entdate, bas.bas_resno, bas_pos.cd_engnm bas_pos,
                         bas_dut.cd_engnm bas_dut, bas_dept.dept_eng bas_dept, to_char(sysdate, 'yyyy')||'-p-'||LPAD(CERI_SEQ_ybg.nextval,4,0) as ceri_no
