...
namespace ybgSub003
{
    public partial class UserControl1: UserControl
    {
        ...
        public UserControl1()
        {
            ...
            ct_dept_upp.SelectedValueChanged += InputData_TextChanged;
            ...
            ct_dept_upp.Validated += Input_Validation_Check;
            ct_dept_sdate.Validated += Input_Validation_Check;
            ct_dept_edate.Validated += Input_Validation_Check;
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.BindByName = true;
                cmd.Parameters.Add("dept_code", OracleDbType.Varchar2).Value = "%" + q_dept_code.Text + "%";
                cmd.Parameters.Add("dept_name", OracleDbType.Varchar2).Value = "%" + q_dept_name.Text + "%";
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    cmd.Parameters.Add("dept_upp", OracleDbType.Varchar2).Value = Utility.GetCode((string)row.Cells["dept_upp"].Value);
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            {
                SetError(ct_dept_name, "부서명칭(원형)을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            {
                SetError(ct_dept_names, "부서명칭(축약)을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            //*---------------------------------------------------------------------------------------------------------
           /* if (Utility.Check_Date_IsEmpty(ct_dept_sdate.Text))
            {
                SetError(ct_dept_sdate, "생성일자를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else if (Utility.Check_Date_YYYYMMDD(ct_dept_sdate.Text) == false)
            {
                SetError(ct_dept_sdate, "생성일자 날짜형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_dept_sdate, errorProvider1);
            }*/
            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_YYYYMMDD(ct_dept_edate.Text) == false)
            {
                SetError(ct_dept_edate, "폐기일자 날짜형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_dept_edate, errorProvider1);
            }
            //*---------------------------------------------------------------------------------------------------------
        }
